...
from web.models import UserLogs, SrcVulnerabilitie, SrcSubDomain, SrcUrls, SrcPorts, SrcWeb, SrcVuln, SrcDomain
...
def src_count():
    '''统计数据库数量'''
    url_count = SrcWeb.query.count()
    subdomain_count = SrcSubDomain.query.count()
    vuln_count = SrcVuln.query.count()
    name_count = SrcDomain.query.count()
    dict1 = {}
    dict1['url_count'] = url_count
    dict1['subdomain_count'] = subdomain_count
    dict1['vuln_count'] = vuln_count
    dict1['name_count'] = name_count
    return dict1
